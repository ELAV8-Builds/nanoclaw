...
export class SlackChannel implements Channel {
  ...
  private statusMessages = new Map<string, string>(); // jid → message ts
    ...
    this.app.event('message', async ({ event }) => {
      ...
      if (files && files.length > 0) {
        const descriptions = files.map((f) => {
          ...
          return `[Attached file: "${name}" (${type}) — ${url}]`;
        });
        fileContext = '\n' + descriptions.join('\n');
      }

      if (!msg.text && !fileContext) return;
  ...
  // Slack has no typing indicator API, so we post a status message on start
  // and delete it once the first real response is sent.
  async setTyping(jid: string, isTyping: boolean): Promise<void> {
    ...
    if (isTyping) {
      try {
        ...
        if (res.ts) this.statusMessages.set(jid, res.ts);
      } catch (err) {
        logger.debug({ jid, err }, 'Failed to post Slack status message');
      }
    } else {
      const ts = this.statusMessages.get(jid);
      if (ts) {
        this.statusMessages.delete(jid);
        try {
          await this.app.client.chat.delete({ channel: channelId, ts });
        } catch (err) {
          logger.debug({ jid, err }, 'Failed to delete Slack status message');
        }
      }
    }
  }
